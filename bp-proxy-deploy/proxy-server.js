...
// Enable CORS for all routes (updated for Vite)
app.use(cors({
...
app.post('/auth/bp/oauth2/token', async (req, res) => {
  const USE_REAL_AUTH = process.env.USE_REAL_BP_API === 'true';
...
app.listen(PORT, () => {
  ...
  const useRealAPI = process.env.USE_REAL_BP_API === 'true';
  console.log(`🎭 Mode: ${useRealAPI ? 'REAL BP API' : 'MOCK DATA'}`);
  
  if (useRealAPI) {
    console.log(`🔑 Client ID: ${BP_CONFIG.clientId.substring(0, 8)}...`);
    console.log(`🌐 Auth URL: ${BP_CONFIG.authUrl}`);
    console.log(`🌐 API URL: ${BP_CONFIG.apiUrl}`);
  }
